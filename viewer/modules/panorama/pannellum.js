...
window.libpannellum = (function (E, g, p) {
    function Ba(K) {
        ...
        function ka(a, d, c, g, k, h) {
            ...
            this.x = g;
            this.y = k;
            this.path = h.replace("%s", d).replace("%l", c).replace("%x", g).replace("%y", k);
        }
        function Ja(a, d, g, p, k) {
            ...
            -4 == t || 4 == t
                ? (h = !1)
                : ((t = h[1] + u[1] + x[1] + e[1]), (h = -4 == t || 4 == t ? !1 : 4 != h[2] + u[2] + x[2] + e[2]));
            if (h) {
                h = d.vertices;
                u = h[0] + h[3] + h[6] + h[9];
                x = h[1] + h[4] + h[7] + h[10];
                ...
                u = Math.abs(u);
                d.diff = Math.acos(Math.sin(g) * Math.sin(e) + Math.cos(g) * Math.cos(e) * Math.cos(u));
                u = !1;
                ...
                if (d.level < c.level) {
                    ...
                    t = 0.5;
                    if (d.x == u || d.y == u) t = 1 - m.tileResolution / (m.tileResolution + l);
                    var y = 1 - t,
                        e = [],
                        s = t,
                        z = t,
                    ...
                    if (l < m.tileResolution)
                        ...
                        } else d.x != u && d.y == u && ((F = s = 0.5), "l" == d.side || "r" == d.side) && (B = D = 0.5);
                    x <= m.tileResolution &&
                        (d.x == u && ((s = 0), (F = 1), "l" == d.side || "r" == d.side) && ((D = 0), (B = 1)),
                        d.y == u && ((z = 0), (A = 1), "d" == d.side || "u" == d.side) && ((D = 0), (B = 1)));
                    l = [
                        h[0],
                        h[1],
                        h[2],
                    ...
                    (d.x == u && x <= m.tileResolution) ||
                        ...
                        (l = new ka(l, d.side, d.level + 1, 2 * d.x + 1, 2 * d.y, m.fullpath)),
                        e.push(l));
                    (d.x == u && x <= m.tileResolution) ||
                        ...
                        (l = new ka(l, d.side, d.level + 1, 2 * d.x + 1, 2 * d.y + 1, m.fullpath)),
                        e.push(l));
                    (d.y == u && x <= m.tileResolution) ||
                        ((l = [
                            h[0] * t + h[9] * y,
                        ...
                        (l = new ka(l, d.side, d.level + 1, 2 * d.x, 2 * d.y + 1, m.fullpath)),
                        e.push(l));
        ...
        this.init = function (L, d, Ca, I, k, h, e, u) {
            function x(a) {
                if (E) {
                    ...
                    c[1] *= 255;
                    c[2] *= 255;
                    for (var b = 0; b < d; b++) (h[b++] = c[0]), (h[b++] = c[1]), (h[b++] = c[2]);
            ...
            if ("cubemap" == z)
                for (t = 0; 6 > t; t++)
                    0 < m[t].width
                        ? (y === p && (y = m[t].width),
                          y != m[t].width &&
                              console.log("Cube faces have inconsistent widths: " + y + " vs. " + m[t].width))
                        : (E = !0);
            ("cubemap" == z &&
                0 !== (y & (y - 1)) &&
                ...
                (a || (a = A.getContext("experimental-webgl", { alpha: !1, depth: !1 })),
            ...
            if (
                ...
                var D = function () {
                        0 < this.width
                            ? ($ === p && ($ = this.width),
                              $ != this.width &&
                                  console.log("Fallback faces have inconsistent widths: " + $ + " vs. " + this.width))
                            : (E = !0);
                        S++;
                ...
                for (t = 0; 6 > t; t++)
                    (h = new Image()),
                        ...
                        (h.onload = k),
                        (h.onerror = D),
                        (h.src = "multires" == z ? I.replace("%s", Q[t]) + "." + m.extension : m[t].src);
                x($);
            } else {
            ...
            if (ga !== p) {
                ...
                var e = ga[1],
                    ...
                    t =
                        ...
                        Math.cos(g) * (Math.cos(k) * Math.cos(d) + Math.sin(e) * Math.sin(k) * Math.sin(d)),
                    E = -Math.sin(g) * Math.sin(e) + Math.cos(g) * Math.cos(e) * Math.sin(d);
                g =
                    Math.cos(e) * Math.cos(k) * Math.sin(g) +
                    Math.cos(g) * (-Math.cos(d) * Math.sin(k) + Math.cos(k) * Math.sin(e) * Math.sin(d));
                g = Math.asin(Math.max(Math.min(g, 1), -1));
                d = Math.atan2(E, t);
                k = [
                    Math.cos(u) * (Math.sin(e) * Math.sin(k) * Math.cos(x) - Math.cos(k) * Math.sin(x)),
                    Math.cos(u) * Math.cos(e) * Math.cos(x),
                    Math.cos(u) * (Math.cos(k) * Math.sin(e) * Math.cos(x) + Math.sin(x) * Math.sin(k)),
                ];
            ...
            if (a || ("multires" != z && "cubemap" != z)) {
                if ("multires" != z)
                    ...
                    for (
                        e = 1;
                        e < m.maxLevel &&
                        a.drawingBufferWidth > m.tileResolution * Math.pow(2, e - 1) * Math.tan(l / 2) * 0.707;
                    ...
                    c.currentNodes = [];
                    e = "fbudlr".split("");
                    for (k = 0; 6 > k; k++) (u = new ka(wa[k], e[k], 1, 0, 0, m.fullpath)), Ja(h, u, g, d, l);
                ...
                for (
                    k = $ / 2,
                        ...
                        d = "perspective(" + l + ") translateZ(" + l + ") rotateX(" + g + "rad) rotateY(" + d + "rad) ",
                        l = Object.keys(s),
                        g = 0;
                    6 > g;
                    g++
                    ...
                    if ((h = R.querySelector(".pnlm-" + l[g] + "face")))
                        (h.style.webkitTransform = d + s[l[g]]), (h.style.transform = d + s[l[g]]);
        };
            ...
            La = (function () {
                function c() {
                    ...
                    var d = function () {
                        ...
                        S.length ? ((c = S.shift()), e.loadTexture(c.src, c.texture, c.callback)) : (l[g++] = e);
                    };
    ...
    function Ba(s, l) {
        ...
        function ua(a) {
            if (ha && H) {
                ...
                a = Q(a);
                var c =
                    (((180 * (Math.atan((xa / n) * 2 - 1) - Math.atan((a.x / n) * 2 - 1))) / Math.PI) * b.hfov) / 90 +
                    Oa;
        ...
        function wa(a, b) {
            ...
            n &&
                b &&
                ((ba = "undefined" !== typeof performance && performance.now() ? performance.now() : Date.now()), G());
        }
        function ga() {
            if (H) {
                var a = !1,
                ...
                var e = ((d - ba) * b.hfov) / 1700,
                    e = Math.min(e, 1);
                r[0] && !0 === b.keyboardZoom && (x(b.hfov + (0.8 * w.hfov + 0.5) * e), (a = !0));
                r[1] && !0 === b.keyboardZoom && (x(b.hfov + (0.8 * w.hfov - 0.2) * e), (a = !0));
                ...
                0 < e &&
                    ...
                    r[2] || r[3] || r[6] || r[7] || O.pitch || (b.pitch += w.pitch * e * a),
            ...
            if (!Za)
                if ((Fa(), Qa && clearTimeout(Qa), ha || !0 === X)) requestAnimationFrame(ca);
                    ...
                    0 < a
                        ...
                        : 0 <= b.autoRotateInactivityDelay && Z && ((b.autoRotate = Z), da.lookAt(Ga, p, ra, 3e3), G());
                }
        }
        function Fa() {
            var a;
            if (H) {
                ...
                if (b.avoidShowingBackground) {
                    var c = b.hfov / 2,
                        e = (180 * Math.atan2(Math.tan((c / 180) * Math.PI), f.width / f.height)) / Math.PI;
                    b.vaov > b.haov
        ...
        function Ca(a) {
            ...
            if ((90 >= a.yaw && -90 < a.yaw && 0 >= h) || ((90 < a.yaw || -90 >= a.yaw) && 0 >= h))
                ...
                var m = C.getCanvas(),
                    p = m.clientWidth,
                    m = m.clientHeight,
                    f = [((-p / k) * l * c) / h / 2, ((-p / k) * (f * d - c * g * e)) / h / 2],
        ...
        function I(a) {
                ...
                if (l.default.hasOwnProperty(f))
                    if ("strings" == f)
                        for (c in l.default.strings)
                            l.default.strings.hasOwnProperty(c) && (b.strings[c] = D(l.default.strings[c]));
            ...
            if (null !== a && "" !== a && l.scenes && l.scenes[a]) {
                ...
                for (f in d)
                    if (d.hasOwnProperty(f))
                        if ("strings" == f)
                            for (c in d.strings) d.strings.hasOwnProperty(c) && (b.strings[c] = D(d.strings[c]));
            ...
            for (f in l)
                if (l.hasOwnProperty(f))
                    if ("strings" == f)
                        for (c in l.strings) l.strings.hasOwnProperty(c) && (b.strings[c] = D(l.strings[c]));
        ...
        function k(a) {
            if ((a = a ? a : !1) && "preview" in b) {
                ...
                sa.style.backgroundImage = "url('" + F(c).replace(/"/g, "%22").replace(/'/g, "%27") + "')";
                M.appendChild(sa);
            }
            var c = b.title,
                e = b.author;
                ...
                if (b.hasOwnProperty(d))
                    switch (d) {
                        ...
                        case "autoLoad":
                            !0 === b[d] &&
                                C === p &&
                                ((q.load.box.style.display = "inline"), (v.load.style.display = "none"), oa());
                            break;
                        case "showZoomCtrl":
                            v.zoom.style.display = b[d] && !1 != b.showControls ? "block" : "none";
                            break;
            ...
            a && (c ? (b.title = c) : delete b.title, e ? (b.author = e) : delete b.author);
        }
        function h() {
            if (H && !Na)
                if (Aa)
        ...
        function u(a) {
            ...
            b.avoidShowingBackground &&
                ...
                (d = Math.min(
                    d,
                    (360 * Math.atan((Math.tan(((b.maxPitch - b.minPitch) / 360) * Math.PI) / a.height) * a.width)) /
                        Math.PI
                )));
            return d;
        }
